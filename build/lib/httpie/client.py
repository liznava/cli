...
def make_default_headers(args: argparse.Namespace) -> HTTPHeadersDict:
    ...
    config = Config()
        ...
        if config.get('json', {}).get('simplified_accept', False):
        # adding to use simple accept header for application/json
        default_headers['Accept'] = JSON_CONTENT_TYPE
        else:
        # keeping previous part
        default_headers['Accept'] = JSON_ACCEPT
        if args.json or (auto_json and args.data):
        default_headers['Content-Type'] = JSON_CONTENT_TYPE
